...
- 히어로 배너/풀블리드 이미지: 없음 — Work는 그리드로 즉시 시작. 페이지 제목을 반복해 적지 않는다(내비게이션이 이미 말해준다).
...
| `≤ 780px` | 사이드바 해제 — 워드마크 왼쪽, 메뉴 아이콘 오른쪽. Work / Biography / Contact와 DE·EN은 아이콘을 눌러야 열리는 패널 안으로 들어간다. Work 1열, 홈 히어로 4:3, 링크 터치 영역 44px 이상 |
| `≤ 400px` | 여백 16px까지 축소 |
